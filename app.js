const APP_BUILD_VERSION="20260511goldv91_desktop_staff_clean";
    ...
    <div class="splash-shell">
      <div class="splash-brand">
        <div class="splash-icon-frame">
          <img class="splash-icon" src="icons/icon-192.png?v=20260511goldv91_desktop_staff_clean" onerror="this.style.display='none'">
        </div>
      ...
      <button id="splashRetryBtn" class="btn ghost splash-retry" onclick="location.href='/?v=20260511goldv91_desktop_staff_clean&reset=1'">Retry / Reset</button>
    </div>
  </div>`;
}
  ...
  try {
    app.innerHTML = `<div class="splash-ultra">
      ...
      <div class="splash-shell splash-error">
        <div class="splash-brand"><div class="splash-icon-frame"><img class="splash-icon" src="icons/icon-192.png?v=20260511goldv91_desktop_staff_clean" onerror="this.style.display='none'"></div></div>
        <h1>${esc(title || 'Startup error')}</h1>
        <p>${esc(msg || 'Something went wrong while loading the app.')}</p>
        <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap;margin-top:20px">
          <button class="btn" onclick="location.href='/?v=20260511goldv91_desktop_staff_clean&reset=1'">Retry</button>
...
async function cleanLogout(){
  ...
  try{ landingView('home'); }catch(e){ location.href = location.pathname + '?v=20260511goldv91_desktop_staff_clean#home'; }
}
...
/* GOLD V91: targeted desktop sidebar scrollbar + staff legacy top cleanup */
(function(){
  function isStaffProfileRoute(){
    try { return current && current.role === 'staff' && String(currentRoute||'') === 'profile'; }
    catch(e){ return false; }
  }

  function setBodyClass(){
    try{ document.body.classList.toggle('staff-profile-clean', !!isStaffProfileRoute()); }catch(e){}
  }

  function removeStaffLegacyTopOnly(){
    try{
      setBodyClass();
      if(!isStaffProfileRoute()) return;
      const view=document.getElementById('view');
      if(!view) return;
      ...
      Array.from(view.children).forEach(el=>{
        ...
        const isLegacyHero =
          /Staff CPD Dashboard|Track your CPD progress, DHP renewal readiness, and quick actions/i.test(txt) &&
          /Welcome,/i.test(txt);
        const isLegacySmart =
          /Smart Alert|DHP renewal attention/i.test(txt);
        const isLegacyQuickButtons =
          /Print Summary/i.test(txt) && /CPD Activities/i.test(txt) && /My Profile/i.test(txt);
        const isOldProfileDuplicate =
          /Staff CPD Dashboard/i.test(txt) && /Total CPD/i.test(txt) && /DHP Renewal/i.test(txt) && !/My Profile\s*Update your DHP/i.test(txt);

        if(isLegacyHero || isLegacySmart || isLegacyQuickButtons || isOldProfileDuplicate){
          el.remove();
        }
      });

      Array.from(view.querySelectorAll('.smart-alert,.smart-alert-row,.staff-hero,.profile-hero,.legacy-smart-alert')).forEach(el=>{
        const txt=(el.textContent||'');
        if(/Smart Alert|DHP renewal attention|Staff CPD Dashboard/i.test(txt)){
          el.remove();
        }
      });
    }catch(e){}
  ...
  if(oldRenderRoute && !window.__goldV91RenderPatched){
    window.__goldV91RenderPatched = true;
    renderRoute = function(){
      const out = oldRenderRoute.apply(this, arguments);
      requestAnimationFrame(removeStaffLegacyTopOnly);
      setTimeout(removeStaffLegacyTopOnly, 120);
      return out;
    };
  }

  window.goldV91CleanStaffTop = removeStaffLegacyTopOnly;
  setTimeout(removeStaffLegacyTopOnly, 150);
})();
